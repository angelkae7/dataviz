...
export class MaladiesComponent implements OnInit {
  ...
  public chartOptionsAgeChapitre:  any;
  public chartOptionsAgeBloc:  any;
  ...
  constructor(private apiService: ApiService, private cdr: ChangeDetectorRef) {
    ...
    this.chartOptionsCauses = {
        ...
        events: {
          dataPointSelection: (event: any, chartContext: any, config: any) => {
            ...
            if (selectedYear && selectedChapter) {
              ...
              this.loadAgeChapitre(selectedChapter, selectedYear);
              this.clearGraph();
            }
          }
        }
      ...
      title: {
        text: 'Nombre de décès | Cause : "Chapitre" / An'
      },
      plotOptions: {
        heatmap: {
          colorScale: {
    ...
    this.chartOptionsBloc = {
        ...
        events: {
          ...
          dataPointSelection: (event: any, chartContext: any, config: any) => {
            ...
            if (selectedBloc) {
              ...
              this.loadAgeBloc(selectedBloc, this.selectedYear!);
            }
      ...
      plotOptions: {
        bar: {
          borderRadius: 6,
          horizontal: true, // Indique que les barres doivent être horizontale
          dataLabels: {
    ...
    this.chartOptionsAgeChapitre = {
      ...
      title: {
        text: "Répartition des morts selon l'Âge et la cause (Chapitre)",
        align: 'left',
    ...
    this.chartOptionsAgeBloc = {
      ...
      title: {
        text: `Répartition des morts selon l'Âge et la cause (Bloc)`,
  ...
  loadAgeBloc(blocs : string, annee:number):void {
    this.selectedYear = annee;
    this.causes_bloc = blocs;
    console.log(annee, this.causes_bloc)
    this.apiService.getAgeGroupPercentageByBloc(blocs, annee).subscribe(
      (datas: AgeData[]) => {
        console.log('Age Data:', datas);
  
        if (datas && datas.length > 0) {  
          
          const blocs = datas.map((d: AgeData) => d['classe_age']);
          const seriesData = datas.map((d: AgeData) => d.total_deces);
          console.log( 'seriesData', seriesData)
          //undefined
          console.log('blocs',blocs);
          this.chartOptionsAgeBloc.series = seriesData;
          this.chartOptionsAgeBloc.labels = blocs;
  ...
  //
  loadAgeChapitre(chapitre : string, annee:number):void {
    this.selectedYear = annee;
    this.causes_chapitre = chapitre;
    console.log(annee, this.causes_chapitre)
    this.apiService.getAgeGroupPercentageByCause(chapitre, annee).subscribe(
      (datas: AgeData[]) => {
        console.log('Age Data:', datas);
  
        if (datas && datas.length > 0) {  
          
          const chapitres = datas.map((d: AgeData) => d['classe_age']);
          const seriesData = datas.map((d: AgeData) => d.total_deces);
          console.log( 'seriesData', seriesData)
          //undefined
          console.log('chapitre',chapitres);
          this.chartOptionsAgeChapitre.series = seriesData;
          this.chartOptionsAgeChapitre.labels = chapitres;
          this.updateAgeChapitreTitle();
          this.updateOptionsBlocTitle();
  ...
  clearGraph(): void {
    ...
    this.chartOptionsAgeBloc.series = [];
    this.chartOptionsAgeBloc.labels = [];
  }

  updateAgeChapitreTitle() {
    const yearValue = this.selectedYear ? this.selectedYear.toString() : 'No year selected';
    this.chartOptionsAgeChapitre.title.text = `Décédés de ${this.causes_chapitre} pour l'Année ${yearValue}`;
  }

  updateOptionsBlocTitle() {
    const yearValue = this.selectedYear ? this.selectedYear.toString() : 'No year selected';
    this.chartOptionsBloc.title.text = `Décédés de ${this.causes_chapitre} pour l'Année ${yearValue}`;
  }
}
